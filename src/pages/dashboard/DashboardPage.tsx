...
import type { MonthlyData, CategoryData, DashboardStats } from '@/types';
...
export function DashboardPage() {
  ...
  const [recentTransactions, setRecentTransactions] = useState<(Income | Expense & { type: 'income' | 'expense' })[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const allRecent = [
        ...recentIncomes.slice(0, 3).map(i => ({ ...i, type: 'income' as const })),
        ...recentExpenses.slice(0, 3).map(e => ({ ...e, type: 'expense' as const })),
      ]
      ...
      setRecentTransactions(allRecent as any);
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard
              title="Month Income"
              amount={stats.currentMonthIncome}
              icon={<TrendingUp />}
              variant="income"
            ...
            <StatCard
              ...
              amount={stats.currentMonthExpenses}
              icon={<TrendingDown />}
              variant="expense"
            ...
            <StatCard
              title="Month Savings"
              amount={stats.currentMonthSavings}
              icon={<PiggyBank />}
              variant="savings"
              subtitle={stats.currentMonthSavings >= 0 ? 'Saved this month' : 'Over budget'}
            />
            <RateCard
              title="Savings Rate"
              rate={stats.savingsRate}
              subtitle={stats.savingsRate >= 20 ? 'Excellent!' : stats.savingsRate >= 10 ? 'Good' : 'Needs attention'}
              icon={<Percent />}
            />
          </div>
        )}
        ...
        <div className="divide-y divide-surface-100 dark:divide-surface-700">
          {loading ? (
            ...
            recentTransactions.map((tx) => {
              ...
              const incTx = tx as Income & { type: 'income' };
              const expTx = tx as Expense & { type: 'expense' };
                ...
                <div key={tx.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-surface-50 dark:hover:bg-surface-750 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-surface-900 dark:text-surface-100 truncate">
                      {isIncome ? (incTx.source || incTx.description) : expTx.description}
                    </p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-surface-400">{formatDate(tx.date)}</span>
                      {!isIncome && (
                        <Badge variant="default" size="sm">{expTx.category}</Badge>
                      )}
                      {!isIncome && expTx.is_shared && (
